...
from runtime.installer.state import (  # noqa: E402
    InstallerState,
    IntegrationPaths,
    clear_state,
...
from scripts.integration_config import (  # noqa: E402
    integration_present,
...
class InstallOptions:
    ...
    def package_spec(self) -> str:
        return ".[tui]" if self.install_tui_extra else "."
...
def resolve_backend(options: InstallOptions) -> str:
    if options.backend != "auto":
        if options.backend == "uv":
            ensure_uv(bootstrap=options.bootstrap_uv)
        elif options.backend == "pipx" and not shutil.which("pipx"):
...
def install_binary(options: InstallOptions) -> str:
    check_python()
    ...
    if backend == "uv":
        ...
        command = ["pipx", "install", "--force", "--editable", f"{checkout}[tui]" if options.install_tui_extra else str(checkout)]
    else:
...
def run_install(options: InstallOptions) -> InstallerState:
    ...
    capiforge_bin = install_binary(options)
...
def run_update(options: InstallOptions, *, state: InstallerState | None = None) -> InstallerState:
    ...
    merged = InstallOptions(
        ...
        backend=existing.backend if existing.backend != "unknown" else options.backend,
    ...
    capiforge_bin = install_binary(merged)
    repo_root = Path(existing.repo_root)
    node_home = Path(existing.node_home)
    if repo_root.exists():
    ...
    existing.backend = resolve_backend(merged)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common.add_argument("--checkout-root", default=str(REPO_ROOT))
