...
The system MUST allow `claimed` and `in_progress` only while a valid active claim exists. If the claim is released or expires, the task MUST leave active claimed execution before another agent proceeds.
